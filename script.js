...
function bumpCompactInput(input, delta) {
  ...
  const isAssignmentDimensionInput = input === assignmentRowCountInput || input === assignmentColumnCountInput;
  const currentValue = isSimplexDimensionInput
    ...
    : isTransportDimensionInput
      ...
      : isAssignmentDimensionInput
        ? clampAssignmentDimension(input.value)
...
function formatAssignmentMinima(prefix, values) {
  return values.map((value, index) => `${prefix}${index + 1} = ${formatValue(value)}`).join(" · ");
}
...
function getAssignmentZeroMatchingSize(costs) {
  const rowCount = costs.length;
  const columnCount = costs[0]?.length ?? 0;
  ...
  for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
    const visited = Array.from({ length: columnCount }, () => false);

    if (tryMatch(rowIndex, visited)) {
      matches += 1;
    }
  }
...
function matricesMatch(left, right) {
  ...
  for (let rowIndex = 0; rowIndex < left.length; rowIndex += 1) {
    if ((left[rowIndex]?.length ?? 0) !== (right[rowIndex]?.length ?? 0)) {
      return false;
    }

    for (let columnIndex = 0; columnIndex < left[rowIndex].length; columnIndex += 1) {
      if (Math.abs(left[rowIndex][columnIndex] - right[rowIndex][columnIndex]) >= EPSILON) {
        return false;
      }
    }
  }
